...
import { openAliceStore } from './storage';
...
export async function runExchange(message: string, log: (line: string) => void): Promise<void> {
  ...
  const persistent = await openAliceStore();
  const runNumber = await persistent.start(log);
  const bobUserId = `bob-${runNumber}`;
  const relay = inMemoryRelay();
  const aliceId = await relay.registerDevice('alice', { encryptedDeviceName: new ArrayBuffer(0) });
  const bobId = await relay.registerDevice(bobUserId, { encryptedDeviceName: new ArrayBuffer(0) });
  ...
  try {
    ...
    const alice = await createSignalProtocolClient({
      ...
      adapters: { storage: persistent.store, relay },
      logger: {},
    });
    log("Alice identity is ready. Publish her prekeys.");
    clients.push(alice);
    await alice.syncToServer();
    log("Create Bob and publish his prekeys.");
    const bob = await createSignalProtocolClient({
      identity: { userId: bobUserId, deviceId: bobId },
    ...
    await deliver(alice, bob, bobUserId, bobId, message);
    await deliver(bob, alice, 'alice', aliceId, `Received: ${message}`);
    await persistent.complete(log);
