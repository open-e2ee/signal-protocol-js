...
`IndexedDbSignalProtocolStore` implements the full `SignalProtocolLocalStore` contract for web
...
keys, device records, and retry metadata. The adapter encrypts every record
with AES-256-GCM.
...
- review dependency and service-worker update paths.
...
not persist. It never means a partial record. The application should free
space or request more from the platform, then retry.

The storage contract suites run against this adapter in real Chromium,
Firefox, and WebKit on every change to the source repository. Three more
suites run beside them.

- A multi-tab suite drives concurrent revision-checked writes through two
  live connections on one origin. That is the state a deployed application
  enters when a user opens a second tab.
- An interruption suite destroys a tab while a write is in flight. It then
  asserts that the reopened store is readable, and that every atomic
  security commit landed all-or-nothing.
- A storage-pressure suite clamps the origin's quota in Chromium and
  exhausts it. It asserts the typed rejection, intact prior state, and a
  clean retry once space frees. Beside it, a jest suite drives quota-shaped
  backend failures through every write path in the adapter.

A soak runner drives thousands of full open/write/read/close cycles through
the adapter in one Chromium page. It fails if per-cycle latency drifts
upward across the run. It also fails if memory drifts upward, including
ArrayBuffer backing stores, sampled after forced garbage collection.

The contract suites are the same modules the jest gate runs. Every assertion
targets this adapter's contract. The engines are the environment the adapter
must honor its promises in, not the subject of the tests.
