...
export class ExpoSecureStoreSignalProtocolSecretVault implements ISignalProtocolLocalSecretVault {
  ...
  async setSecret(name: string, value: Uint8Array): Promise<void> {
    await SecureStore.setItemAsync(name, bytesToBase64(value), SIGNAL_PROTOCOL_SECURE_STORE_OPTIONS);
  }

  async deleteSecret(name: string): Promise<void> {
    await SecureStore.deleteItemAsync(name, SIGNAL_PROTOCOL_SECURE_STORE_OPTIONS);
  }
}
