...
This installable Convex component owns the complete relay backend: account and
device registration, identity and prekey storage, message queues, sealed-sender
delivery, retry requests, provisioning, sender certificates, canonical
encrypted group state, anonymous group authorization, credential issuance, and
group-send endorsements. The app owns only its
authentication-to-protocol-identity mapping and thin public wrappers.
...
The `env` forwarding is load-bearing: Convex components are isolated from the
app's environment variables, so without it the component cannot see the
deployment's `OE_GROUPS_SERVER_SECRET` and every group, credential-issuance,
and sender-certificate call fails at runtime. The messaging, device, key, and
provisioning namespaces work without the secret.
...
The component derives **no** membership map from group state. Group reads and
writes are authorized entirely by zero-knowledge presentations, so `groups`,
`groupChanges`, and `groupSnapshots` never name an account, and there is no
`getGroupMembers` endpoint anywhere in the relay surface. Membership is
local-first: each client resolves fan-out recipients from its own decrypted
group state and passes their user IDs with the send
(`SendOptions.groupMemberUserIds`); the relay expands users to devices via
`getActiveDevices`.
...
`messages` owns identified and sealed delivery, multi-recipient fan-out, retry
queues, and message/device lookup reads. `devices` owns registration, removal,
connection state, and presence. `keys` owns identity provisioning and
compare-and-swap rotation, prekey upload, atomic one-time-key consumption, and
key metadata. `certificates` issues sender certificates for registered
account/device identities. `provisioning` owns the linked-device session state
machine through acknowledgment or rollback.
...
send material, preserving delivery without an authenticated application
session. Rejections are structured `ConvexError` data, and the relay adapter
decides whether to fall back to identified delivery by reading
`data.code === 'UNAUTHORIZED'` — not by matching on the message text. A
deployment that wraps or re-throws component rejections must preserve `code`,
or a sealed send that should have retried on the identified path will surface
as a hard failure instead.

A recipient's unidentified-access key is registered as a side effect of the
zero-knowledge profile-key credential flow
(`issueProfileKeyCredentialMutation`). Until a recipient has completed that
flow, access-key-authorized sealed sends to them are rejected and the client
...
PNI: credentials are matched by ACI-or-PNI alias, so a constant PNI issued to
every account would let any authenticated user act on any PNI-addressed pending
membership. An absent PNI must be absent.
...
session TTL. A link that is never acknowledged within that window is rolled
back by the cron: the device it created is deleted together with all of its
key material, queued messages, and heartbeats. Teardown matches the device's
`linkedAt` stamp recorded at completion, so a device legitimately
re-registered into a freed slot is never reaped by a stale session.
...
push**, before any of your code runs. A stored row carrying a `groupId`, or
one whose `messageType` is `plaintext_content`, therefore fails the deploy —
and no migration mutation can fix it, because the deploy that would ship the
mutation is the deploy that fails. The order has to be: clear first, deploy
second.

Messages are transient by design. The table has a seven-day TTL and an hourly
cleanup cron, and undelivered messages are re-requested by clients through the
retry path, so clearing it is a normal operation rather than data loss.

Check whether the deployment has anything to clear — still on the previous
version, since the table lives inside the component rather than the app:
...
than your dev one. A deployment that has never relayed a group message and
never had a caller pass `plaintext_content` has no affected rows and needs no
...
stdin, and prints two labelled base64 roots — the serialized group trust root
and the Ed25519 sender-certificate root. Pin both in the client build; see
...
They are pinned in different places and are not interchangeable — the group
...
fetch either from a relay at runtime, since a relay that chooses the root it is
validated against can mint certificates for any sender.

Both are derived from the same deployment secret, so rotating
...
and sends fall back to identified delivery — messages still arrive, but the
sender is disclosed to the relay.

## Group metadata privacy

The relay cannot read message contents, cannot derive membership from group
state, and no longer sees a group identifier on the two channels that used to
carry one.
...
`groupId:senderUserId:senderDeviceId:timestamp`, so the group and the sender's
device were readable straight out of the ciphertext column — and, worse than a
column, off every message in flight to any relay on the path. Sealed sender did
not help: it hides the frame from the relay but not from the rest of the path,
and it is disabled until the trust root is exported (see above), so group
traffic takes the identified path anyway. Distribution identifiers are now
opaque random UUIDs carrying no group, sender, device, or clock, as the
reference implementation uses, as the sender-keys specification now requires.
...
is gone. What replaces it is a `sender_key` message type, which tells the
receiver to decrypt the payload as a framed `SenderKeyMessage` and nothing more:
the receiver reads the opaque distribution identifier out of the frame and
resolves the group against its own sender key store. No group identifier
...
- The frame's `distributionUuid` is unencrypted and is stored verbatim inside
  `messages.ciphertext` — the paragraph above says as much. It is opaque, but
  it is stable for the life of a sender key, and nothing rotates keys on a
...
attributes — but it can still partition delivery pairs into unlabeled groups,
and any one of those channels suffices. That is a real improvement on a
plaintext `groupId`, which handed over a stable, meaningful, cross-send label;
it is not anonymity of the partition. The `sender_key` type is also a
single-column filter isolating group traffic from pairwise traffic, which the
old design did not offer — group messages used to hide among `ciphertext` rows.
...
granularity, and the `deviceId` arguments are routing selectors chosen by the
caller rather than authenticated claims. Concretely, any session for an account
can read and delete **any** of that account's device queues
(`getPendingMessages`, `markDelivered`), send as any of its device IDs
(`send.senderDeviceId`), and upload or rotate key material for any of its
devices (`keys`).

This is the intended model — a compromised session already holds the account —
but it means the component provides no isolation *between* an account's own
devices. Deployments that need device isolation must issue device-scoped
sessions and enforce the binding in their own `identify` wrapper; the component
cannot do it while its identity has no device field.
