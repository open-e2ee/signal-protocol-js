...
export class S3ObjectStore implements SignalProtocolRemoteObjectStore {
  ...
  public readonly reconcileUpload?: S3ObjectStoreBroker["reconcileUpload"];

  constructor(config: S3ObjectStoreConfig) {
    this.broker = config.broker;
    if (config.broker.reconcileUpload) {
      this.reconcileUpload = async (input) => {
        validateObjectId(input.objectId);
        return this.broker.reconcileUpload!(input);
      };
    }
