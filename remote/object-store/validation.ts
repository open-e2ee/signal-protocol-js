...
/** Check a broker receipt without exposing its object identity or digest in errors. */
export function matchesUploadReceipt(
  value: unknown,
  expected: { objectId: string; contentLength: number; digest: Uint8Array },
): boolean {
  ...
  return (
    ...
    receipt.contentLength === expected.contentLength &&
    receipt.digest instanceof Uint8Array &&
    receipt.digest.length === 32 &&
    expected.digest.length === 32 &&
    receipt.digest.every((byte, index) => byte === expected.digest[index])
  );
}
...
function requireSecureUrl(value: unknown, field: string): string {
  ...
  if (
    ...
    !(parsed.protocol === "http:" && loopbackHost)
  ) {
    throw new TypeError(
...
function requireExpiration(value: unknown, field: string): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value <= 0) {
    throw new TypeError(
      `${field} must be a positive Unix timestamp in milliseconds`,
    );
  }
  return value;
}
...
export function validateUploadRequest(
  ...
  requireExpiration(input.preparedAt, "preparedAt");
